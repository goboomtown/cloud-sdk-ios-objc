...
- (instancetype) init {
    self = [super init];
    if (self) {
        _host = @"https://api.goboomtown.com/api/v3";
        _username = @"";
        _password = @"";
        _accessToken= @"";
        _verifySSL = YES;
...
#pragma mark - Set API keys

- (void) setAPIToken:(NSString *)token
{
    [self setApiKey:token forApiKeyIdentifier:kAPIToken];
}

- (void) setAPIPrivateKey:(NSString *)key
{
    [self setApiKey:key forApiKeyIdentifier:kAPIPrivateKey];
}
